...
# This is one of the most idiotic things on R. No simple reference to pwd.
setwd(system("pwd", intern = T) )
cat('EXAMPLE 0: The stochastic neoclassical growth model\n')


dummy<-readline('Hit ENTER key when ready...')
...
# for k(t-1):
BB = matrix(c( - (1 - betta*(1-delta))*(1-rho),
       K_bar/(betta * C_bar) ,
       rho                             ))

# For [ c(t), r(t), y(t) ]
CC = rbind(c(     0,   -1,    0),   # Equ. 1)
          c(-1,    0,    0),   # Equ. 2)
           c(0,    0,   -1 )) # Equ. 3)

# For z(t):
DD = matrix(c( (1 - betta*(1-delta)),
       (1 + delta * K_bar/C_bar),
       1                        ))
...
JJ = t(matrix(c(    -eta,     1 ,  0     )))

# For [ c(t), r(t), y(t) ]
KK = t(matrix(c(   eta,   0,    0  )))
