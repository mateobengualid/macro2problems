...
cat('EXAMPLE 2: Hansen RBC model with a monopolistically competitive\n')
cat('           intermediate sector, extending Hansen, G., \n')
cat('           "Indivisible Labor and the Business Cycle,"\n')
cat('           Journal of Monetary Economics, 16 (1985), 281-308.\n')
...
choose_alpha = 0
if (choose_alpha) {
   cat('Hope you have chosen a value for alpha between 0 and 1:\n')
   cat(sprintf('alpha = %8.4f\n',alpha))
   cat('\n')
} else {
   alpha   <- .7
}
...
CC = rbind(c(    -C_bar,      Y_bar,      0,        0,        -I_bar), # Equ. 1)
          c(0,           0,          0,        0,        I_bar),  # Equ. 2)
          c(0,           -1,         1-rho,    0,        0),      # Equ. 3)      
          c(-eta,        1,          -1,       0,        0),      # Equ. 4)
          c(0,           D_bar,      0,        - R_bar,  0 ))   # Equ. 5)
...
JJ = t(matrix(c( -eta,  0,  0,  1,  0)))

KK = t(matrix(c( eta,   0,  0,  0,  0)))
...
cat('Note, that the dynamics are hardly different from the dynamics of exampl1.R\n');
