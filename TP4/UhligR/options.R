...
#
# OPTIONS.R sets the options to be used in all the subroutines.
# It is called by DO_IT.R.  If you use any of the routines
# "by hand", i.e. without calling them via DO_IT.R, you also
# have to execute OPTIONS.R first.
...
#   DISPLAY_LATER = 1;                # = 1: displays warning messages after SOLVE.R
#                                     # DO THIS, IF YOUR CALCULATIONS MYSTERIOUSLY CRASH!
...
#                      # a simulation.  SIM_TRACK_FAC = 1000 is set in OPTIONS.R.
...
#                      # If set to 1, letter size will be manipulated by ENLARGE.R
...
# CHANGING ANY OPTION OTHER THAN THE ONES LISTED ABOVE REQUIRES CHANGING THE FILE OPTIONS.R
...
if (!exists('DISPLAY_IMMEDIATELY')) {
   DISPLAY_IMMEDIATELY = 0; # = 1: displays warning messages immediately.
}
if (DISPLAY_IMMEDIATELY) {
   message = rbind('OPTIONS.R: You will receive warning messages always immediately.       ',
   ...
   message = rbind('OPTIONS.R: You will not receive warning messages immediately.          ',
             '           To turn on immediate Warnings, set DISPLAY_IMMEDIATELY = 1  ',
              '           Warning messages are stored in the array "Warnings"         ')
   print(message,quote=F);
   Warnings = rbind(Warnings,message)
}
message = rbind('OPTIONS.R: There are lots of options to choose from.  Type help options',
           '           to get more information about them!                         ')
if (DISPLAY_IMMEDIATELY) {print(message,quote=F)}
Warnings = rbind(Warnings,message)
if (!exists('DISPLAY_LATER')) {
   DISPLAY_LATER = 0;                # = 1: displays warning messages after SOLVE.R
}
if (!exists('DISPLAY_AT_THE_END')) {
   DISPLAY_AT_THE_END = 1;           # = 1: displays warning messages at the end of all computations.
}
if (!exists('DO_HP_FILTER')) {
   DO_HP_FILTER= 1;    # Set to = 1, if you want moments and simulations with HP-filter.
                       # set to = 0, if you want moments and simulations without HP-filter.
}
if (!exists('HP_LAMBDA')) {            # The lambda parameter for the HP-Filter:
...
if (!exists('N_LEADS_LAGS')) {
   N_LEADS_LAGS = 6;       # calculate autocorr. table up to N_LEADS_LAGS-1 leads and lags.
}
if (!exists('DO_ENLARGE')) {
   DO_ENLARGE = 0; # =1, if you need large letter sizes for overheads, say.
                   # If set to 1, letter size will be manipulated by
                   # the program ENLARGE.R
}
if (!exists('DO_THICK_LINES')) {
   DO_THICK_LINES = 1; # If you set DO_ENLARGE = 1, then you will also get thick lines with DO_THICK_LINES = 1;
}
if (!exists('PRINT_FIG')) {
   PRINT_FIG  = 0; # Set = 1, if you want to sent figures directly to printer
}
if (!exists('DO_COLOR_PRINT')) {
...
if (!exists('DO_MOMENTS')) {
   DO_MOMENTS = 0; # Turn on, if you want to do
                   # fourier-transforms-based calculations of moments
}
if (DO_MOMENTS) {
   message = rbind('OPTIONS.R: Note, that I calculate moments, which takes time. If you    ',
              '           do not want to have them calculated, set DO_MOMENTS = 0     ')
   if (DISPLAY_IMMEDIATELY) {print(message,quote=F)}
   Warnings = rbind(Warnings,message)   
} else {
   message = rbind('OPTIONS.R: Note, that I will not calculate moments.                    ',
...
if (DO_SIMUL) {
   message = rbind('OPTIONS.R: Note, that I calculate simulations, which takes time.       ',
              '           If you do not want to have them calculated, set DO_SIMUL = 0')
   if (DISPLAY_IMMEDIATELY) {print(message,quote=F)}
   Warnings = rbind(Warnings,message)   
} else {
   message = rbind('OPTIONS.R: Note, that I will not calculate simulations.                ',
...
if (DO_SIMUL | DO_MOMENTS) {
   if (DO_HP_FILTER) {
      if (HP_LAMBDA >= 1e+11) {
         line1 = sprintf('OPTIONS.R: I will use the HP-Filter with lambda=%12.4e.          ',HP_LAMBDA)
      }
	if (HP_LAMBDA >= 1000) {
         line1 = sprintf('OPTIONS.R: I will use the HP-Filter with lambda=%12.0f.          ',HP_LAMBDA)
      } 
	if (HP_LAMBDA >= 1) {
         line1 = sprintf('OPTIONS.R: I will use the HP-Filter with lambda=%12.4f.          ',HP_LAMBDA)
      } 
	if (HP_LAMBDA >= .0001) {
         line1 = sprintf('OPTIONS.R: I will use the HP-Filter with lambda=%12.8f.          ',HP_LAMBDA)
      } else {
         line1 = sprintf('OPTIONS.R: I will use the HP-Filter with lambda=%12.4e.          ',HP_LAMBDA)
      }
      message = rbind(as.character(line1),
                 '           If you want to change that, modify DO_HP_FILTER and         ',
                 '           HP_LAMBDA in OPTIONS.R or in your program or by changing    ',
                 '           e.g.HP_LAMBDA now and restarting the calculations with do_it')
      if (DISPLAY_IMMEDIATELY) {print(message,quote=F)}
      Warnings = rbind(Warnings,message)   
   } else {
      message = rbind('OPTIONS.R: I will NOT use the HP-Filter.                               ',
                 '           If you want to change that, modify DO_HP_FILTER and         ',
                 '           HP_LAMBDA in OPTIONS.R or in your program or by modifying   ',
...
if (!exists('IGNORE_VV_SING')) {
   IGNORE_VV_SING = 1; # =1: Ignores, if VV is singular.  
                       # Sometimes useful for sunspots.  Cross your fingers...  
}


#============> options for SOL_OUT.R
DISPLAY_ROOTS = 1;  # Set = 1, if you want to see the roots.



#============> options for IMPRESP.R
if (!exists('DO_PLOTS')) {
   DO_PLOTS   = 1; # if impulse response plots should be made, = 0, if not.
}
if (DO_PLOTS) {
   message = rbind('OPTIONS.R: Note, that I plot impulse responses, which you may dislike. ',
              '           If you do not want to have them plotted, set DO_PLOTS = 0   ')
   Warnings = rbind(Warnings,message)
   message = rbind('OPTIONS.R: For imp.responses you can choose between joint-, sub-, and  ', 
              '           single plots. Try, SIM_JOINT=1, SIM_SUBPLOT=1, SIM_SINGLE=1 ') 
   if (DISPLAY_IMMEDIATELY) {print(message,quote=F)}
   Warnings = rbind(Warnings,message)
} else {
   message = rbind('OPTIONS.R: Note, that I will not plot impulse responses.               ',
...
if (!exists('IMP_SUBPLOT')) { # if impulse respones should be plotted in a subplot 
    IMP_SUBPLOT=0;
}
if (!exists('IMP_JOINT')) { # if all responses should be plotted in one graph, =0, if not 
    IMP_JOINT=1;
}
 
if (!exists('IMP_SINGLE')) { # if each response should be plotted in a single graph, =0, if not 
    IMP_SINGLE=0;
}
if (!exists('IMP_SUB_FONT')) {  # Size of the title and labels in the subplots
    IMP_SUB_FONT=6;
}
if (!exists('INIT_DATE')) {
...
if (!exists('IMP_SELECT')) {
   IMP_SELECT = 1:(m_states+n_endog+k_exog);
                #  a vector containing the indices of the variables to be plotted
}
if (!exists('DO_NO_ZERO_RESPONSE')) {
   DO_NO_ZERO_RESPONSE =  0;
                # Set = 1 to avoid plotting responses near zero
}
if (!exists('ZERO_RESPONSE_LEVEL')) {
   ZERO_RESPONSE_LEVEL =  0.0001;
                # Defining near zero to be the interval 
                # [-ZERO_RESPONSE_LEVEL,ZERO_RESPONSE_LEVEL]
}
if (!exists('DO_SCALE_AXIS')) {
   DO_SCALE_AXIS =  0;
                #  if the y-axis should be scaled according to different set of variables
}
if (!exists('DO_GIVEN_AXIS_SCALE')) {
   DO_GIVEN_AXIS_SCALE =  0;
                # Scale according to a given axis scale, see next variable.
                # Only use = 1, if you are familiar with scaling axis in MATLAB.
                # see help axis for more information.
}
if (!exists('AXIS_SCALE')) {
   AXIS_SCALE =  matrix(c(0,HORIZON,-1,1))
                # The argument for the axis(.) command, 
                # normally a vector of length 4 = [xmin,xmax,ymin,ymax].
}
if (!exists('AXIS_SELECT')) {
   AXIS_SELECT = 1:(m_states+n_endog+k_exog);
...
if (!exists('SIM_RANDOM_START')) {
   SIM_RANDOM_START = 1; 
                    # = 1, if you want a random start, = 0, if you want predetermined start
}
if (!exists('SIM_X_START')) {
   SIM_X_START = matrix(0,nrow=m_states,ncol=1)
                    # if predetermined start, SIM_X_START should be a vector of length
                    # m, setting the initial values for the endogenous state variables x.
}
if (!exists('SIM_Z_START')) {
   SIM_Z_START = matrix(0,nrow=k_exog,ncol=1) 
                    # if predetermined start, SIM_X_START should be a vector of length
                    # k, setting the initial values for the exogenous state variables z.
}
SIM_DISCARD = 100;  # Length of initial discarded observations, if random start.
if (!exists('SIM_GIVEN_EPS')) {
   SIM_GIVEN_EPS = 0; # set = 1, if you want to choose the shocks epsilon yourself.
                      # In that case, declare them beforehand as
                      # given_eps 
                      # which needs to be a matrix of size k_exog x SIM_LENGTH
...
if (!exists('SIM_TRACK_N')) {
   SIM_TRACK_N = 1;   # Prints a little message at the start of each simulation.
                      # ONLY RELEVANT FOR SIM_MODE = 2.
}
if (!exists('SIM_TRACK_LENGTH')) {
   SIM_TRACK_LENGTH = 1; # Prints a little message every SIM_TRACK_FAC date of
                      # a simulation.  
}
SIM_TRACK_FAC = 1000;
...
#============> options for SIM_OUT.R
if (!exists('SIM_GRAPH')) {
   SIM_GRAPH = 1;   # Set to = 1 to see plots of the simulated series.
}
if (SIM_GRAPH & DO_SIMUL) {
   message = rbind('OPTIONS.R: Note, that I plot simulations, which you may dislike.       ',
              '           If you do not want to have them plotted, set SIM_GRAPH=0    ')
   Warnings = rbind(Warnings,message)   
   message = rbind('OPTIONS.R: For simulations you can choose between joint-, sub-, and    ', 
              '           single plots. Try, SIM_JOINT=1, SIM_SUBPLOT=1, SIM_SINGLE=1 ') 
   if (DISPLAY_IMMEDIATELY) {print(message,quote=F)}
   Warnings = rbind(Warnings,message)   
} else   {
   message = rbind('OPTIONS.R: Note, that I will not plot simulations.                     ',
...
if (!exists('SIM_SUBPLOT')) { # if simulated series should be plotted in a subplot, =0, if not
    SIM_SUBPLOT=0;
}
if (!exists('SIM_JOINT')) { # if all simulated series should be plotted in one graph, =0, if not 
    SIM_JOINT=1;
} 
if (!exists('SIM_SINGLE')) { # if each simulated series should be plotted in one graph, =0, if not 
    SIM_SINGLE=0;
} 
if (!exists('SIM_SUB_FONT')) {  # Size of the title and labels in the subplots
    SIM_SUB_FONT=6;
}
if (!exists('SIM_SELECT')) {
   SIM_SELECT = 1:(m_states+n_endog+k_exog); 
                      # Selecting the variables for the graphics of simulations.
}
if (!exists('SIM_DATE0')) {
...
SIM_DO_DISP3    = 1;# Set to = 1 to see printout of the vector of variances.
   # For the graphics output in SIM_OUT.R:
...
if (!exists('HP_SELECT')) {
   HP_SELECT  = 1:(m_states+n_endog+k_exog) 
      ...
      message = rbind('OPTIONS.R: I will proceed with your chosen HP_SELECT.  If that leads to',
                 '           a crash in moments.R or mom_out.R or to not a complete      ',
...
#============> options for MOM_OUT.R
if (!exists('DO_HP_GRAPH')) { # To plot spectral densities 
    DO_HP_GRAPH=1;
}
if (DO_HP_GRAPH & DO_MOMENTS) {
   message = rbind('OPTIONS.R: Note, that I plot spectral densities, which you may dislike.',
              '           If you do not want to have them plotted, set DO_HP_GRAPH =0 ')
   Warnings = rbind(Warnings,message)   
   message = rbind('OPTIONS.R: For moments you can choose between joint-, sub-, and        ', 
              '           single plots. Try, SIM_JOINT=1, SIM_SUBPLOT=1, SIM_SINGLE=1 ') 
   if (DISPLAY_IMMEDIATELY) {print(message,quote=F)} 
   Warnings = rbind(Warnings,message)   
} else {
   message = rbind('OPTIONS.R: Note, that I will not plot spectral densities.              ',
...
DO_DISP3    = 1;    # Set to = 1 to see printout of the vector of variances.
   # For the graphics output in MOM_OUT.R:
MOM_TXT_MARKER = round(N_GRIDPOINTS/24); 
                    # Where to put the labels on the graph, 
                    # a number between 1 and N_GRIDPOINTS/2.                                    
if (!exists('MOM_SUBPLOT')) { # if spectral densities should be plotted in a subplot 
    MOM_SUBPLOT=0;
}
if (!exists('MOM_JOINT')) { # if all spectral densities should be plotted in one figure, =0, if not 
    MOM_JOINT=1;
} 
if (!exists('MOM_SINGLE')) { # if spectral densities should be plotted in separate figures
    MOM_SINGLE=0;
}
if (!exists('MOM_SUB_FONT')) {  # Size of the title and labels in the subplots
    MOM_SUB_FONT=7;
}
if (!exists('MOM_PLOT_RAW')) { # if you like non-HP-filtered spectral density plots
    MOM_PLOT_RAW=0;
}
