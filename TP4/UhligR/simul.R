...
if (DO_HP_FILTER) {
   HP_mat = rbind(cbind(1+HP_LAMBDA, -2*HP_LAMBDA, HP_LAMBDA,              matrix(0,1,SIM_LENGTH-3)),
             cbind(-2*HP_LAMBDA,1+5*HP_LAMBDA,-4*HP_LAMBDA,HP_LAMBDA, matrix(0,1,SIM_LENGTH-4)),
                           matrix(0,SIM_LENGTH-4,SIM_LENGTH),
              cbind(matrix(0,1,SIM_LENGTH-4),HP_LAMBDA,-4*HP_LAMBDA,1+5*HP_LAMBDA,-2*HP_LAMBDA),     
              cbind(matrix(0,1,SIM_LENGTH-3),          HP_LAMBDA,   -2*HP_LAMBDA, 1+HP_LAMBDA  ))
   for (i in 3:(SIM_LENGTH-2)) {
...
if (SIM_TRACK_N & (SIM_MODE == 2)) {
   message = rbind('SIMUL.M: I will inform you about starting each new simulation.         ',
...
if (SIM_TRACK_LENGTH & (SIM_LENGTH >= SIM_TRACK_FAC)) {
   message = rbind('SIMUL.M: I will inform you about my progress as I calculate each       ',
...
Sig_fac = t(Sig_fac)     # to get the lower triangular Cholesky factor
if (SIM_MODE == 2) {
   autcor_sum = matrix(0,n_select+1,(2*N_LEADS_LAGS-1))
   autcor_sqr = matrix(0,n_select+1,(2*N_LEADS_LAGS-1))
   covmat_sum = matrix(0,n_select,n_select)
   covmat_sqr = matrix(0,n_select,n_select)
   stdvec_sum = matrix(0,n_select,1)
   stdvec_sqr = matrix(0,n_select,1)
   n_sim = SIM_N_SERIES
   message = rbind('SIMUL.M: I will now calculate simulation-based moments.                ',
   ...
   message = rbind('SIMUL.R: You have set SIM_MODE to a nonsensical values: allowed are    ',
...
for (sim_index in 1:n_sim) {
   if ((SIM_MODE == 2) & SIM_TRACK_N) {
      sprintf('SIMUL.R: starting simulation %5d out of %5d...',sim_index,n_sim)
   } # end if simmode=2, simtrackn
   if (SIM_RANDOM_START) {
      xyz <- matrix(0,m_states+n_endog+k_exog,1)
      if (SIM_GIVEN_DISCARD & (SIM_MODE != 2)) {
         if (sim_index == 1) {
            message = rbind('SIMUL.R: I am using your given shocks epsilon.  You should have defined',
         ...
         sim_discard_eps = Sig_fac%*%matrix(c(rnorm(n=k_exog*SIM_DISCARD,mean=0,sd=1)),nrow=k_exog,ncol=SIM_DISCARD)
      }
      for (t in 1:SIM_DISCARD) {
         xyz =II_contemp%*%(II_lag%*%xyz + 
                             matrix(c(matrix(0,m_states+n_endog,1),sim_discard_eps[,t])))
      }
   } else {
      xyz = II_contemp%*%(II_lag%*%matrix(c(SIM_X_START,matrix(0,n_endog+k_exog,1)))+
                             matrix(c(matrix(0,m_states+n_endog,1),SIM_Z_START)))
   }
   sim_xyz <- matrix(0,m_states+n_endog+k_exog,SIM_LENGTH)
   if (SIM_GIVEN_EPS & (SIM_MODE != 2)) { # CHANGE IN 2002: Replaced 'and' with '&'
      ...
      sim_eps = Sig_fac%*%matrix(rnorm(k_exog*SIM_LENGTH,mean=0,sd=1),k_exog,SIM_LENGTH)
   } # end if
   sim_xyz[,1] = xyz
   for (t in 2:SIM_LENGTH) {
      if (SIM_TRACK_LENGTH & (t == floor(t/SIM_TRACK_FAC)*SIM_TRACK_FAC)) {
         sprintf('SIMUL.R:      Simulation date t = #10d out of #10d...',t,SIM_LENGTH)
      }
      xyz =II_contemp%*%(II_lag%*%xyz +
                          matrix(c(matrix(0,m_states+n_endog,1),sim_eps[,t])))
      sim_xyz[,t] = xyz
   } # end for
   sim_raw <- sim_xyz
   if (DO_HP_FILTER) {
         if (SIM_TRACK_LENGTH | (SIM_TRACK_N & (SIM_MODE == 2))) {
            cat('SIMUL.M:      Doing the HP-Filter...\n')
         }
      sim_xyz <- t(sim_raw) - solve(HP_mat)%*%t(sim_raw)
      sim_xyz = t(sim_xyz)
   } # end if dohpfilter
   covmat_one = matrix(0,n_select,n_select)
   for (j in 1:n_select) {
      sim_ser = matrix(1,n_select,1)%*%sim_xyz[SIM_SELECT[j],]
	covmat_j = colSums( t(sim_ser)*t(sim_xyz[SIM_SELECT,]) )
      # covmat_j = sum( t(sim_ser).* t(sim_xyz[SIM_SELECT,]) )
      covmat_one[,j] = t(covmat_j)
   } # end for
   ...
   sim_gnp = cbind(matrix(0,n_select,(N_LEADS_LAGS-1)),
              matrix(1,n_select,1)%*%t(matrix(sim_xyz[GNP_INDEX,])),
              matrix(0,n_select,(N_LEADS_LAGS-1)))
   autcor_one = matrix(0,n_select+1,(2*N_LEADS_LAGS-1))
   for (j in 1:(2*N_LEADS_LAGS-1)) {
	tempmat <- t(sim_gnp[,j:(j-1+SIM_LENGTH)])*t(sim_xyz[SIM_SELECT,])
	autcor_j <- matrix(NA,nrow(t(matrix(colSums(tempmat)/(SIM_LENGTH - abs(N_LEADS_LAGS - j))))),
			        ncol(t(matrix(colSums(tempmat)/(SIM_LENGTH - abs(N_LEADS_LAGS - j))))))
	for (row in 1:nrow(t(matrix(colSums(tempmat)/(SIM_LENGTH - abs(N_LEADS_LAGS - j))))))  {
		for (col in 1:ncol(t(matrix(colSums(tempmat)/(SIM_LENGTH - abs(N_LEADS_LAGS - j)))))) {
		autcor_j[row,col] <-
		t(matrix(colSums(tempmat)/(SIM_LENGTH - abs(N_LEADS_LAGS - j))))[row,col]/
		(t(stdvec_one) * stdvec_one[GNP_INDEX])[row,col]
		} # end col
	} # end row
      ...
      autcor_one[,(2*N_LEADS_LAGS-j)] = matrix(c(t(autcor_j),(N_LEADS_LAGS-j)))
   } # end for
   if (SIM_MODE == 2) {
